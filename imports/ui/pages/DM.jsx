...
export const DM = () => {
  ...
  const addCharToEncounter = (selectedCharacterName) => {
    const isAlreadyInEncounter = encounterList.includes(selectedCharacterName);

    if (!isAlreadyInEncounter) {
      // Add the selected character to the encounterList
      setEncounterList((prevList) => [...prevList, selectedCharacterName]);
    ...
    <>
      ...
      <div style={{ display: "flex" }}>
          ...
          autoFocus
					...
					/>
					<Button onClick={handleAccessRoom}>Access Room</Button>
				</div>
				...
				>
							{encounterList.length === 0 ? (
								<Typography>No characters in encounter</Typography>
							) : (
								<List sx={{width: "70%"}}>
									{encounterList.map((character, index) => (
										<ListItem sx={{border: 1, margin: 0.25}}
											button
											key={index}
										>
											<ListItemText
                        primary={`${index + 1}. ${character}`}
                        ...
                        sx = {{margin: 0.5}}
												...
												<Button
                        sx = {{margin: 0.5}}
												onClick={ () => handleRemoveEncounter(character)}
												type="submit"
												variant="contained"
												color="error"
												edge="end"
